export default async function update_event(data) {
  const {
    name,
    descripcion,
    fecha,
  ...
  if (nameError) throw new Error(`Nombre inválido: ${nameError}`);

  const descError = this.validator.validateDescription(descripcion, 'evento');
  if (descError) throw new Error(`Descripción inválida: ${descError}`);
  ...
  if (costoError) throw new Error(`Costo inválido: ${costoError}`);

  const lugarError = this.validator.validateName(lugar, 'evento');
  if (lugarError) throw new Error(`Lugar inválido: ${lugarError}`);
  ...
  return await this.dbms.executeNamedQuery({
    nameQuery: 'updateEvents', // asegurarse de usar updateEvento, no createEvento
    params: [
      name,
      descripcion,
      fecha,
